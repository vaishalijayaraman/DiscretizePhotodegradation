...
x_num = len(img_grid)
y_num = len(img_grid[0])
x_step = 1/(len(img_grid))
y_step = 1/(len(img_grid[0]))
# print("x_step is: ", x_step)
# print("t_step is: ", t_step)

z_num = int((zf-z0) / z_step)
t_num = int((tf-t0) / t_step)

# CREATE FINAL GRID
final_grid = np.zeros((x_num,y_num,t_num))
final = np.zeros((x_num,y_num))
...
def computeIntensity(prev, j, alpha_pd_star, alpha_deg_star, z_num, ci, cd, cf):
	summation = 0
	for k in range (j, z_num):
		s = ((alpha_pd_star * (ci + (cd/2))) + (alpha_deg_star * ((cf + (cd/2)))))/(z_num)
...
# EXPLICIT METHOD
def explicit(grid, ci_grid, cd_grid, cf_grid, t0, tf, t_step, z0, zf, z_step, alpha_pd_star, alpha_deg_star, z_num, ci, cd, cf):
	for i in range (1, t_num): 
		...
		for j in range(0, z_num):
			mult = prev_ci[j] + (prev_cd[j]/2)
			mult_0 = ci_0 + (cd_0/2)

			int_star = computeIntensity(prev, j, alpha_pd_star, alpha_deg_star, z_num, prev_ci[j], prev_cd[j], prev_cf[j])

			ci = prev_ci[j] + (-2 * int_star * mult**2) * t_step

			cd = prev_cd[j] + (((-2 * int_star * (mult_0 - 2*(mult))) * mult) * t_step)

			diff_f = get_diff_f(z0, zf, z_step, z_num, cf_grid, i, j)
...
# IMPLICIT METHOD
def implicit(grid, ci_grid, cd_grid, cf_grid, t0, tf, t_step, z0, zf, z_step, alpha_pd_star, alpha_deg_star, z_num, ci, cd, cf, masked):
	for i in range (1, t_num): 
		prev = grid[(i-1)]
		...
		for j in range(0, z_num):
			...
			Df_ci = lambda x: 1 + (4 * t_step * int_star) - (2 * t_step * (x**2) * (alpha_pd_star/z_num) * int_star) + (2 * t_step * int_star * prev_cd[j]) - (2 * t_step * x * prev_cd[j] * (alpha_pd_star/z_num) * int_star)
			...
			Df_cd = lambda x: 1 + (2 * t_step * int_star * ((-1 * (prev_ci[j] + (x/2))) + (0.5 * (((ci_0) + (cd_0/2)) - (2 * prev_ci[j]) - x)))) - (2 * t_step * int_star * ((((ci_0) + (cd_0/2)) - (2 * prev_ci[j]) - x) * (prev_ci[j] + x/2)) * ((alpha_pd_star + alpha_deg_star)/(2*z_num))) 
			...
			Df_cf = lambda x: 1 - (2 * t_step * ((prev_ci[j] + (prev_cd[j]/2)) * (prev_ci[j] + (prev_cd[j]/2) - ci_0 - (cd_0/2))) * int_star * (alpha_deg_star/z_num))
			...
			value = computeIntensity(prev, j, alpha_pd_star, alpha_deg_star, z_num, value_ci, value_cd, value_cf)
...
# DIFFUSION TERM
def get_diff_f(z0, zf, z_step, z_num, cf_grid, i, j):
	v = 0
	vals = cf_grid[i-1]
...
# SET GRID
grid = np.arange(z_num*t_num*1.0).reshape((t_num, z_num))
grid.fill(0.0)
grid[0] = 1.0

ci_grid = np.arange(z_num*t_num*1.0).reshape((t_num, z_num))
ci_grid.fill(0.0)
ci_grid[0] = 1.0

cd_grid = np.arange(z_num*t_num*1.0).reshape((t_num, z_num))
cd_grid.fill(0.0)

cf_grid = np.arange(z_num*t_num*1.0).reshape((t_num, z_num))
cf_grid.fill(0.0)
...
for row in range(x_num):
	for col in range(y_num):
		#check if masked
		if img_grid[row][col] == 0: #black is mask
			masked = True
		else:
			masked = False
		...
		grid = np.transpose(g)
		# print("grid: ", grid)
		final_grid[row][col] = grid[:, t_num-1] #verify accuracy!!!
		final[row][col] = grid[:, 1][0]
# print("final: ", final)
print("final_grid: ", final_grid)

# GRAPH 
plt.imshow(final, interpolation='none', cmap=cm.Blues)
plt.show()
...
n = int((len(img_grid)*len(img_grid[0]))/10)
print("n is: ", n)
...
mesh = triangle.build(info, max_volume=0.95, min_angle=25)
...
cdVals = np.zeros((len(tri_list),2))
for tp in range (len(tri_list)):
    tripts = []
